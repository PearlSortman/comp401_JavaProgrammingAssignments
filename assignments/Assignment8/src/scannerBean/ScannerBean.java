...
import commands.EndToken;
import commands.Minus;
import commands.Move;
import commands.Number;
import commands.NumberTokenInterface;
...
@PropertyNames({ "originalString", "largeArray", "compactArray", "arrayIndex", "startIndex", "stopIndex", "isQuote",
		"mySubstring" })
...
public class ScannerBean implements TokenInterface, ScannerBeanInterface {
	...
	// public String getMyTestInput() { return getMyTestInput(); }
	...
	public void scan() {
		...
		while (++stopIndex < originalString.length()) {
			...
			// Checks for beginning quotes and once found, creates substring of
			// everything inside the quotes. Also adds quote to largeArray.
			...
			// If the token is anything other than a quote, we find the
			// substring here then determine what type of token it is.
		...
		// Once the entire string has been scanned and tokens added to the
		// largeArray.
		// This loop copies those tokens into the compactArray with the exact
		// size of the originalString.
		for (int index = 0; index < arrayIndex; index++) {
			compactArray[index] = largeArray[index];
			largeArray[index] = null;
		}

		// Reset arrayIndex to 0 so the next input string creates a new array
		// instead of building upon the previous one.
		arrayIndex = 0;
	}

	// Private method to analyze all substrings that are not quotes. During this
	// process, tokens are added to largeArray once token type determined.
	private void analyzeTokens(String mySubstring) {
		...
		else if (mySubstring.equalsIgnoreCase("approach")) {
			TokenInterface approach = new Approach(mySubstring);
			approach.setOriginalString(mySubstring);
			largeArray[arrayIndex++] = approach;
		}
		...
		// Tokenize any word other than a known command.
			...
			NumberTokenInterface number = new Number(mySubstring);
